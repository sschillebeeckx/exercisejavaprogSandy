...
public class CompanySession extends Session {
    ...
    public CompanySession(Course course, LocalDate startDate, Company location, Person instructor, Company organizer) {
        super(course, startDate, location, instructor);
        this.organizer = organizer;
    }

    public CompanySession(Course course, LocalDate startDate, Company location, Person instructor, Company organizer, int numberOfParticipants) {
    ...
    public double calculatePrice() throws PriceTooHighException {
        ...
        if (totalPrice>4500) throw new PriceTooHighException(DecimalUtils.format(totalPrice) + " for the " + this.getCourse() + " course is too expensive.");
        return totalPrice;
    }
